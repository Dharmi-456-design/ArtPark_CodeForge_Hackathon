      ...
      <motion.div variants={container} initial="hidden" whileInView="show" viewport={{ once: true }} className="relative flex flex-col lg:flex-row gap-8 mb-16 z-10">
        ...
        {[
          ...
          <motion.div key={i} variants={item} className="flex-1">
            <div className="w-14 h-14 rounded-full bg-slate-50 dark:bg-[#07080f] border-2 border-indigo-500 flex items-center justify-center font-black text-xl mb-4 shadow-[0_0_15px_rgba(99,102,241,0.3)] shadow-indigo-500/30 text-slate-900 dark:text-white" style={{ fontFamily: "'Syne', sans-serif" }}>
              {step.n}
            </div>
            <h3 className="text-2xl font-black mb-3 text-slate-900 dark:text-white" style={{ fontFamily: "'Syne', sans-serif" }}>{step.t}</h3>
            <p className="text-sm text-slate-400 leading-relaxed">{step.d}</p>
          </motion.div>
        ))}
      </motion.div>

      <motion.div variants={item} initial="hidden" whileInView="show" viewport={{ once: true }} className="p-8 rounded-full bg-white dark:bg-slate-900/40 backdrop-blur-md border border-slate-200 dark:border-white/10 flex flex-wrap lg:flex-nowrap items-center justify-between gap-4 shadow-sm dark:shadow-none">
